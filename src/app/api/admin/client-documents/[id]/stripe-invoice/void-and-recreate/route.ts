...
import {
  clearClientDocumentStripeInvoice,
  getClientDocumentById,
  setClientDocumentStripeInvoice,
} from "@/lib/db";
...
export async function POST(_req: Request, ctx: RouteParams) {
  ...
  const oldId = doc.stripeInvoiceId?.trim();
  if (!oldId) {
    return NextResponse.json(
      {
        error:
          "No Stripe invoice on file. Use “Create Stripe invoice” first, or use that action to add an invoice.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    const existingInv = await stripe.invoices.retrieve(oldId);
    ...
    if (existingInv.status === "draft") {
      await stripe.invoices.del(oldId);
    } else if (
      existingInv.status === "open" ||
      existingInv.status === "uncollectible"
    ) {
      await stripe.invoices.voidInvoice(oldId);
    }
  } catch (e) {
    console.warn("[stripe-invoice void-and-recreate] void old", oldId, e);
    return NextResponse.json(
      {
        error:
          "Could not void the previous invoice in Stripe. Check the Stripe Dashboard or try again.",
      },
      { status: 502 },
    );
  }
  await clearClientDocumentStripeInvoice(id);
  ...
  const invoiceDescription = [
    "Ritualmaker floral proposal — event order (revised)",
    memoParts.join(" · "),
  ].join("\n");
